function [new_exit_cells, B_cells_trial] = GC_cycle( B_cells_trial, cycle_number, nb_cycle_max, conc, activation_energy, threshold_energy, p_mut, p_CDR, p_FR_lethal, p_recycle, t_cell_selection, overlap, nb_Ag, energy_scale, p_CDR_lethal, p_CDR_silent, kappa, sigma, mu, nb_max_B_cells)
...
daughters2 = division_and_mutation(daughters1, activation_energy, threshold_energy, p_mut, p_CDR, p_FR_lethal, overlap, p_CDR_lethal, p_CDR_silent, kappa, sigma, mu, nb_Ag );
...
daughters2 = select(daughters2, nb_Ag, energy_scale, overlap, conc, activation_energy, t_cell_selection);
%disp(['size after selection ' num2str(size(B_cells_trial,1))]);

%%RECYCLE
%% randomly pick exit_cells from the selected b_cells.
[ daughters2, new_exit_cells ] = recycling(daughters2, p_recycle);
%[ B_cells_trial ] = recycling(B_cells_trial, p_recycle);
%disp(['size after recycling ' num2str(size(B_cells_trial,1))]);

if size(B_cells_trial,1) <= nb_max_B_cells && cycle_number <= nb_cycle_max
